...
Things that are still assumptions or unconfirmed and should be checked
...
- [ ] `opportunities/` — one real posting (Osgoode JD research assistant, Aug 2026; deadline 13 Sept 2026, start 1 Oct — remove once filled); the *study* (funded graduate positions 2027–28) and *participate* (annual community calls) entries are real but say "details soon / deadline TBA" — fill in when known; a fourth *participate* entry links the email-list sign-up form. `projects/` — placeholders removed; one real project (Refugee Law Lab Portal). Add projects as they are approved.
- [ ] Events: the 22 Sept 2026 launch entry ("AJRRN Town Hall: Kick-Off Event") has no registration link (`link: ""`); the Zoom joining details were published on 2026-09-21 and removed again on 2026-09-23. Decide how attendees will be given the link (mailing list, direct invitation) and add it here if it should be public.
...
- [x] Email list: sign-up linked from the Contact page and Opportunities → Participate (plain link to the York form; no embeds). The form is currently English only — add translated forms if wanted.
- [ ] Optional: social links — the site intentionally has none; decide whether to add plain links (no embeds).
