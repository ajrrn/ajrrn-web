...
- [ ] `opportunities/` — one real posting (Osgoode JD research assistant, Aug 2026; deadline 13 Sept 2026, start 1 Oct — remove once filled); the *study* (funded graduate positions 2027–28) and *participate* (annual community calls) entries are real but say "details soon / deadline TBA" — fill in when known. `projects/` — placeholders removed; one real project (Refugee Law Lab Portal). Add projects as they are approved. Replace them with real items — or delete them, in which case the list pages show a "nothing here yet" note.
