...
- [ ] `people/` — lists the Director, the Stream A lead and the Project Manager (external links, project-manager email shown) under "Team", and the 12 other co-applicants from the SSHRC application under "Research affiliates" (role "Co-applicant", affiliation and an external profile link where one was found). Confirm with each affiliate that they agree to be listed, and how they want their name, affiliation and link shown; replace "Co-applicant" with a more descriptive role if preferred. (Collaborators such as Todd Miller are not listed.)
